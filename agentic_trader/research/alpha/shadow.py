...
def observe_definition(definition, data, as_of):
    ...
    if definition.eligible_symbols and symbol not in definition.eligible_symbols:
        return None
    frame = completed_bars(getattr(data, TIMEFRAME_FIELDS[definition.timeframe]), definition.timeframe, as_of=as_of)
    payload = {
    ...
    completed_at = timestamp + BAR_DURATIONS[definition.timeframe]
