...
on a swept card (for example in the gap before its Telegram strike is delivered) gets the
same `EXPIRED` reply and Re-evaluate offer as a tap-time expiry, from the shared
`_not_pending_reply` path.
