...
| P2 | `/scan` can describe a session/macro-gated run as a completed universe evaluation. | Return a typed scan outcome with the checked policy and actual evaluated/alert counts; share it with CLI/Telegram rendering rather than rechecking gates. Passive readiness already records policy-check progress separately from instrument evaluation. |
