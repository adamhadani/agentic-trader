...
| A3 | Forecast benchmark prerequisite — [PR #43](https://github.com/adamhadani/agentic-trader/pull/43); 78-trial comparison complete; timing/cost screen in progress; combined execution shadow-only | Distinguish forecast components from a fully specified tradable strategy; validate combinations causally. |
...
1. **Forecast objective and simple baselines (current increment).** The frozen
   ...
   freezes next-open targets, QQQ transfer, four cost scenarios and triage criteria
   before payoff evaluation. Keep all existing statistical gates unchanged.
