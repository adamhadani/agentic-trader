...
- `/readyz`: passive, current-run freshness; HTTP 200 when ready, 503 otherwise.
  ...
  connection reset (e.g. the daily-panel worker's periodic `/calendar` poll on a
  pooled connection that idled for minutes), avoiding a spurious one-poll `503`
  from that alone; mutations still never retry automatically.
